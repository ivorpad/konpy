...
```json
{
  ...
  "conventions": [
    {
      "paths": "providers/{providerId}/__init__.py",
      "must": {
        "exportFunctions": [
          { "name": "create_${providerId}_provider" }
        ],
...
For `providers/openai/__init__.py`, the rule expects:
- `create_openai_provider` (the function name)
- `OpenAIProviderConfig` (instead of the default `OpenaiProviderConfig`)
