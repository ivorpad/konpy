...
class PackageJsonDocument:
    """A konpy.json file located inside an installed Python distribution."""
...
class PackageJsonLookupFailure:
    """Why find_package_konpy_json could not locate a distribution's konpy.json."""
...
def find_package_konpy_json(
    value: str,
) -> PackageJsonDocument | PackageJsonLookupFailure:
    """Locate the konpy.json bundled with an installed distribution named `value`."""
...
def _read_top_level_resource(
    *,
    ...
    location_path = f"{package_name}/konpy.json"

    try:
        candidate = resources.files(package_name) / "konpy.json"
...
def _read_dist_info_file(
    *,
    ...
    for package_path in files:
        if package_path.name != "konpy.json":
            continue
