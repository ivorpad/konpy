...
def check_export_types(
    *,
    ...
    """Check that each expected name is exported as a type from the module."""
    ...
    for entry in expected:
        ...
        message = f'Missing export type "{name}"'
        if from_:
            message = f'Missing export type "{name}" from "{from_}"'

        diagnostics.append(
            create_diagnostic(
                file_path=context.path,
                predicate_name="exportTypes",
                message=message,
