"""Payload parsing, path extraction, and prompt/verdict helpers for `konsistent hook`.

Split out of `konsistent.cli.hook` to keep that module under the project's
per-module line limit; `hook.py` re-exports everything here under its own
`__all__` so the public import path (`konsistent.cli.hook`) is unchanged.
"""
...
from konsistent.cli.agent_runner import first_json_object
...
class HookAgent(StrEnum):
    """Verifier agent CLI selectable for `konsistent hook`."""
