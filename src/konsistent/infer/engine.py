"""Engine orchestration for `konsistent infer` (M15 convention mining).
...
a `RawConfigV1`/`konsistent.json`-shaped document.
"""
...
from konsistent.infer.heuristics import (
    annotate_coverage,
    barrel_usage,
    docstring_coverage,
    export_suffix,
...
from konsistent.unused.engine import DEFAULT_INCLUDE as DEFAULT_INFER_INCLUDE
from konsistent.unused.engine import DEFAULT_TEST_GLOBS as DEFAULT_INFER_TEST_GLOBS
...
def build_pack(report: InferReport) -> dict[str, Any]:
    ...
    `konsistent.cli.extract_rules`), never the `RawConfigV1`/`konsistent.json`
